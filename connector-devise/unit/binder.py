...
class WebBinder(Binder):
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.web_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.web_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Devise! Prevent False values
        ...
        # avoid to trigger the export when we modify the `web_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``devise.res.partner`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``devise.res.partner``,
...
@devise
class DeviseModelBinder(WebBinder):
    """
    Bindings are done directly on the binding model.

    Binding models are models called ``devise.{normal_model}``,
    like ``devise.res.partner``.
